    ...
    ngx_array_t *keys, ngx_array_t *passwords)
{
     ...
     *
     * Only the keys the loop above used are considered.  There can be more
     * "ssl_certificate_key" directives than certificates, and the surplus
     * ones are never loaded: reading them here would reject a configuration
     * that works, and matching against a key that is not in the context
     * would hide a certificate that has none.
     */
    ...
    pkey = ngx_palloc(cf->temp_pool, certs->nelts * sizeof(EVP_PKEY *));
    if (pkey == NULL) {
        return NGX_ERROR;
    }
