...
static ngx_int_t ngx_quic_gaps_reserve(ngx_connection_t *c,
...
static ngx_int_t ngx_quic_gaps_written(ngx_quic_buffer_t *qb,
    uint64_t start, uint64_t end);
static ngx_chain_t *ngx_quic_gaps_error(ngx_connection_t *c, ngx_int_t rc);
#if (NGX_DEBUG)
...
static ngx_int_t
ngx_quic_gaps_reserve(ngx_connection_t *c, ngx_quic_buffer_t *qb)
{
    size_t                  n;
    ...
    if (gaps == NULL) {
        ...
        if (gaps) {
            qc->free_gaps = gaps->next;

        } else {

            /*
             * RFC 9000, 21.7.  Stream Fragmentation and Reassembly Attacks
             *
             * the number of gaps is limited in proportion to the stream
             * buffer size, 512 bytes being a conservative minimum useful
             * fragment size
             */

            n = qc->conf->stream_buffer_size / 512 + 4;

            gaps = ngx_palloc(c->pool, sizeof(ngx_quic_gaps_t)
                                       + n * sizeof(ngx_quic_gap_t));
            if (gaps == NULL) {
                return NGX_ERROR;
            }

            gaps->nalloc = n;
            gaps->ranges = (ngx_quic_gap_t *) (gaps + 1);
        }
...
static ngx_chain_t *
ngx_quic_gaps_error(ngx_connection_t *c, ngx_int_t rc)
{
    ngx_quic_connection_t  *qc;
...
static ngx_int_t
ngx_quic_gaps_written(ngx_quic_buffer_t *qb, uint64_t start, uint64_t end)
{
    ngx_uint_t       i, n;
    ...
    ngx_chain_t *in, uint64_t limit, uint64_t offset)
{
    ...
    if (offset > old_last) {
        rc = ngx_quic_gaps_insert(c, qb, old_last, offset);
        if (rc != NGX_OK) {
            return ngx_quic_gaps_error(c, rc);
        }
    }
     ...
     * iterations so the total chain traversal for all in-gap fills is
     * O(n_nodes)
    ...
    for (gi = 0; gi <= nranges && in && limit; gi++) {
        ...
        /* ---- compute [ms, me): sub-range of [offset, frame_end) to write */
        if (gi < nranges) {
     ...
     * ------------------------------------------------------------------ */
    rc = ngx_quic_gaps_written(qb, frame_start, frame_end);
    if (rc != NGX_OK) {
        return ngx_quic_gaps_error(c, rc);
    }
...
ngx_quic_free_buffer(ngx_connection_t *c, ngx_quic_buffer_t *qb)
{
    ...
    if (qb->gaps) {
        ...
        qb->gaps->next = qc->free_gaps;
        qc->free_gaps = qb->gaps;
        qb->gaps = NULL;
    }
}
