    ...
    const unsigned char *data, size_t len, size_t *consumed, void *arg)
{
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_EVENT, c->log, 0,
                   "quic ngx_quic_cbs_send level:%d len:%uz",
                   qc->write_level, len);
    ...
    if (!qc->client_tp_done) {
        ...
        ngx_log_error(NGX_LOG_INFO, c->log, 0, "missing transport parameters");
        return 1;
    }
    ...
    const unsigned char **data, size_t *bytes_read, void *arg)
{
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_EVENT, c->log, 0,
                   "quic ngx_quic_cbs_recv_rcd level:%d", qc->read_level);
...
ngx_quic_cbs_release_rcd(ngx_ssl_conn_t *ssl_conn, size_t bytes_read, void *arg)
{
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_EVENT, c->log, 0,
                   "quic ngx_quic_cbs_release_rcd level:%d len:%uz",
                   qc->read_level, bytes_read);
    ...
    enum ssl_encryption_level_t ssl_level, const uint8_t *data, size_t len)
{
    ...
    if (!qc->client_tp_done) {
        ...
        if (client_params_len == 0) {
            ...
            if (qc->error != NGX_QUIC_ERR_CRYPTO(SSL_AD_MISSING_EXTENSION)) {
                ngx_log_error(NGX_LOG_INFO, c->log, 0,
...
ngx_quic_handshake(ngx_connection_t *c, ngx_uint_t level)
{
    ...
    if (n <= 0) {
        ...
        if (sslerr == SSL_ERROR_WANT_WRITE && qc->error) {
            return NGX_ERROR;
        }
