    ...
    *consumed = 0;
    ...
    if (alpn_len == 0) {
        ...
        ngx_log_error(NGX_LOG_INFO, c->log, 0,
                      "missing ALPN extension in QUIC handshake");
        return 1;
    }
    ...
    const unsigned char *params, size_t params_len, void *arg)
{
    ...
    if (ngx_quic_parse_transport_params(p, end, &ctp, c->log) != NGX_OK
        || ngx_quic_apply_transport_params(c, &ctp) != NGX_OK)
    {
...
ngx_quic_crypto_input(ngx_connection_t *c, ngx_uint_t level)
{
    ...
    if (ngx_quic_crypto_provide(c, level) != NGX_OK) {
        return NGX_ERROR;
    }
...
ngx_quic_init_connection(ngx_connection_t *c)
{
    ...
    if (SSL_set_quic_tls_transport_params(ssl_conn, p, len) == 0) {
        ngx_log_error(NGX_LOG_ALERT, c->log, 0,
                      "SSL_set_quic_tls_transport_params() failed");
        return NGX_ERROR;
    }
#else
