...
#define NGX_CTRL_BAD_METHOD      405
...
ngx_control_init(u_char *addr)
{
    ...
    fd = socket(u.family, SOCK_STREAM, 0);
    ...
    {
        ngx_log_error(NGX_LOG_EMERG, ngx_cycle->log, ngx_socket_errno,
                      "control: setsocketopt(SO_REUSEADDR) failed");
        close(fd);
        return NGX_ERROR;
    }
    ...
    ngx_buf_t *body)
{
    ...
    c->buf = ngx_create_temp_buf(r->pool,
                                 sizeof(response_fmt) + 2 * NGX_INT_T_LEN);
    if (c->buf == NULL) {
        return NGX_ERROR;
    }
    ...
    c->buf->last = ngx_sprintf(c->buf->pos, response_fmt,
                               code, ngx_control_status_text(code), n);
