...
typedef struct {
    const char   *alpha2;
    const char   *alpha3;
} ngx_stream_geoip2_ccode_t;


static ngx_int_t ngx_stream_geoip2_country_variable(ngx_stream_session_t *s,
    ngx_stream_variable_value_t *v, uintptr_t data);
static ngx_int_t ngx_stream_geoip2_org_variable(ngx_stream_session_t *s,
    ngx_stream_variable_value_t *v, uintptr_t data);
static ngx_int_t ngx_stream_geoip2_city_variable(ngx_stream_session_t *s,
    ngx_stream_variable_value_t *v, uintptr_t data);
static ngx_int_t ngx_stream_geoip2_region_name_variable(
    ngx_stream_session_t *s, ngx_stream_variable_value_t *v, uintptr_t data);
static ngx_int_t ngx_stream_geoip2_city_float_variable(
    ngx_stream_session_t *s, ngx_stream_variable_value_t *v, uintptr_t data);
static ngx_int_t ngx_stream_geoip2_city_int_variable(ngx_stream_session_t *s,
    ngx_stream_variable_value_t *v, uintptr_t data);
static ngx_int_t ngx_stream_geoip2_country_code3_variable(
    ngx_stream_session_t *s, ngx_stream_variable_value_t *v, uintptr_t data);
static ngx_int_t ngx_stream_geoip2_area_code_variable(
    ngx_stream_session_t *s, ngx_stream_variable_value_t *v, uintptr_t data);

static ngx_int_t ngx_stream_geoip2_lookup(ngx_stream_session_t *s,
    MMDB_s *mmdb, struct sockaddr *sa, const char **path,
    MMDB_entry_data_s *entry_data);

static ngx_int_t ngx_stream_geoip2_add_variables(ngx_conf_t *cf);
static void *ngx_stream_geoip2_create_conf(ngx_conf_t *cf);
static char *ngx_stream_geoip2_country(ngx_conf_t *cf, ngx_command_t *cmd,
    void *conf);
static char *ngx_stream_geoip2_org(ngx_conf_t *cf, ngx_command_t *cmd,
    void *conf);
static char *ngx_stream_geoip2_city(ngx_conf_t *cf, ngx_command_t *cmd,
    void *conf);
static void ngx_stream_geoip2_cleanup(void *data);

static const char *ngx_stream_geoip2_country_code3_from_code(
    const char *code2);
...
static const char  *ngx_stream_geoip2_path_country_code[] = {
    "country", "iso_code", NULL
};

static const char  *ngx_stream_geoip2_path_country_name[] = {
    "country", "names", "en", NULL
};

static const char  *ngx_stream_geoip2_path_org[] = {
    "autonomous_system_organization", NULL
};

static const char  *ngx_stream_geoip2_path_continent_code[] = {
    "continent", "code", NULL
};

static const char  *ngx_stream_geoip2_path_region[] = {
    "subdivisions", "0", "iso_code", NULL
};

static const char  *ngx_stream_geoip2_path_region_name[] = {
    "subdivisions", "0", "names", "en", NULL
};

static const char  *ngx_stream_geoip2_path_city[] = {
    "city", "names", "en", NULL
};

static const char  *ngx_stream_geoip2_path_postal_code[] = {
    "postal", "code", NULL
};

static const char  *ngx_stream_geoip2_path_latitude[] = {
    "location", "latitude", NULL
};

static const char  *ngx_stream_geoip2_path_longitude[] = {
    "location", "longitude", NULL
};

static const char  *ngx_stream_geoip2_path_dma_code[] = {
    "location", "metro_code", NULL
};


#define NGX_STREAM_GEOIP2_DB_COUNTRY  0
#define NGX_STREAM_GEOIP2_DB_CITY     1
#define NGX_STREAM_GEOIP2_DB_ORG      2
...
static ngx_stream_module_t  ngx_stream_geoip2_module_ctx = {
    ngx_stream_geoip2_add_variables,       /* preconfiguration */
...
static ngx_stream_variable_t  ngx_stream_geoip2_vars[] = {

    { ngx_string("geoip2_country_code"), NULL,
      ngx_stream_geoip2_country_variable,
      (uintptr_t) ngx_stream_geoip2_path_country_code, 0, 0 },

    { ngx_string("geoip2_country_code3"), NULL,
      ngx_stream_geoip2_country_code3_variable,
      0, 0, 0 },

    { ngx_string("geoip2_country_name"), NULL,
      ngx_stream_geoip2_country_variable,
      (uintptr_t) ngx_stream_geoip2_path_country_name, 0, 0 },

    { ngx_string("geoip2_org"), NULL,
      ngx_stream_geoip2_org_variable,
      (uintptr_t) ngx_stream_geoip2_path_org, 0, 0 },

    { ngx_string("geoip2_city_continent_code"), NULL,
      ngx_stream_geoip2_city_variable,
      (uintptr_t) ngx_stream_geoip2_path_continent_code, 0, 0 },
    ...
    { ngx_string("geoip2_region"), NULL,
      ngx_stream_geoip2_city_variable,
      (uintptr_t) ngx_stream_geoip2_path_region, 0, 0 },

    { ngx_string("geoip2_region_name"), NULL,
      ngx_stream_geoip2_region_name_variable,
      (uintptr_t) ngx_stream_geoip2_path_region_name, 0, 0 },

    { ngx_string("geoip2_city"), NULL,
      ngx_stream_geoip2_city_variable,
      (uintptr_t) ngx_stream_geoip2_path_city, 0, 0 },

    { ngx_string("geoip2_postal_code"), NULL,
      ngx_stream_geoip2_city_variable,
      (uintptr_t) ngx_stream_geoip2_path_postal_code, 0, 0 },

    { ngx_string("geoip2_latitude"), NULL,
      ngx_stream_geoip2_city_float_variable,
      (uintptr_t) ngx_stream_geoip2_path_latitude, 0, 0 },

    { ngx_string("geoip2_longitude"), NULL,
      ngx_stream_geoip2_city_float_variable,
      (uintptr_t) ngx_stream_geoip2_path_longitude, 0, 0 },

    { ngx_string("geoip2_dma_code"), NULL,
      ngx_stream_geoip2_city_int_variable,
      (uintptr_t) ngx_stream_geoip2_path_dma_code, 0, 0 },

    { ngx_string("geoip2_area_code"), NULL,
      ngx_stream_geoip2_area_code_variable,
      0, 0, 0 },
...
static ngx_stream_geoip2_ccode_t  ngx_stream_geoip2_country_codes[] = {
...
static const char *
ngx_stream_geoip2_country_code3_from_code(const char *code2)
{
    ngx_stream_geoip2_ccode_t  *cc;

    for (cc = ngx_stream_geoip2_country_codes; cc->alpha2 != NULL; cc++) {
...
static ngx_int_t
ngx_stream_geoip2_lookup(ngx_stream_session_t *s, MMDB_s *mmdb,
...
static ngx_int_t
ngx_stream_geoip2_country_variable(ngx_stream_session_t *s,
    ngx_stream_variable_value_t *v, uintptr_t data)
{
    ...
    if (ngx_stream_geoip2_lookup(s, &gcf->country, s->connection->sockaddr,
                                  path, &entry_data) != NGX_OK)
    {
        goto not_found;
    }
    ...
    p = ngx_pnalloc(s->connection->pool, entry_data.data_size);
    if (p == NULL) {
        return NGX_ERROR;
    }
...
static ngx_int_t
ngx_stream_geoip2_org_variable(ngx_stream_session_t *s,
    ngx_stream_variable_value_t *v, uintptr_t data)
{
    const char                **path = (const char **) data;
    ngx_stream_geoip2_conf_t  *gcf;
    ...
    if (ngx_stream_geoip2_lookup(s, &gcf->org, s->connection->sockaddr,
                                  path, &entry_data) != NGX_OK)
    {
        goto not_found;
    }
...
static ngx_int_t
ngx_stream_geoip2_city_variable(ngx_stream_session_t *s,
    ngx_stream_variable_value_t *v, uintptr_t data)
{
    const char                **path = (const char **) data;
    ngx_stream_geoip2_conf_t  *gcf;
    ...
    if (ngx_stream_geoip2_lookup(s, &gcf->city, s->connection->sockaddr,
                                  path, &entry_data) != NGX_OK)
    {
        goto not_found;
    }
...
static ngx_int_t
ngx_stream_geoip2_region_name_variable(ngx_stream_session_t *s,
    ngx_stream_variable_value_t *v, uintptr_t data)
{
    const char                **path = (const char **) data;
    ngx_stream_geoip2_conf_t  *gcf;
...
static ngx_int_t
ngx_stream_geoip2_city_float_variable(ngx_stream_session_t *s,
    ngx_stream_variable_value_t *v, uintptr_t data)
{
    const char                **path = (const char **) data;
    ngx_stream_geoip2_conf_t  *gcf;
    ...
    p = ngx_pnalloc(s->connection->pool, NGX_INT64_LEN + 5);
    if (p == NULL) {
        return NGX_ERROR;
    }
...
static ngx_int_t
ngx_stream_geoip2_city_int_variable(ngx_stream_session_t *s,
    ngx_stream_variable_value_t *v, uintptr_t data)
{
    const char                **path = (const char **) data;
    ngx_stream_geoip2_conf_t  *gcf;
...
static ngx_int_t
ngx_stream_geoip2_country_code3_variable(ngx_stream_session_t *s,
    ngx_stream_variable_value_t *v, uintptr_t data)
{
    ngx_stream_geoip2_conf_t  *gcf;
    MMDB_entry_data_s          entry_data;
    ...
    if (data == NGX_STREAM_GEOIP2_DB_CITY) {
        if (gcf->city_loaded == 0) {
            goto not_found;
        }
        mmdb = &gcf->city;
    ...
    if (ngx_stream_geoip2_lookup(s, mmdb, s->connection->sockaddr,
                                  ngx_stream_geoip2_path_country_code,
                                  &entry_data) != NGX_OK)
    {
        goto not_found;
    }
    ...
    code3 = ngx_stream_geoip2_country_code3_from_code(code2);
    if (code3 == NULL) {
        goto not_found;
    }

    p = ngx_pnalloc(s->connection->pool, 3);
    if (p == NULL) {
        return NGX_ERROR;
    }
...
static ngx_int_t
ngx_stream_geoip2_area_code_variable(ngx_stream_session_t *s,
    ngx_stream_variable_value_t *v, uintptr_t data)
{
...
static ngx_int_t
ngx_stream_geoip2_add_variables(ngx_conf_t *cf)
{
    ngx_stream_variable_t  *var, *v;

    for (v = ngx_stream_geoip2_vars; v->name.len; v++) {
...
static char *
ngx_stream_geoip2_country(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
...
static char *
ngx_stream_geoip2_org(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ngx_stream_geoip2_conf_t  *gcf = conf;
...
static char *
ngx_stream_geoip2_city(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ngx_stream_geoip2_conf_t  *gcf = conf;
