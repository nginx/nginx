    ...
    ngx_http_upstream_t *u, ngx_uint_t do_write)
{
    ...
    if (u->conf->ignore_input || !r->request_body_no_buffering) {
    ...
    ngx_uint_t from_upstream, ngx_uint_t do_write)
{
    ...
    ngx_int_t                  rc;
    ngx_uint_t                 downstream_buffered, request_pending;
    ngx_chain_t               *out, *ln;
#endif
            ...
            for ( ;; ) {
...
#if (NGX_HTTP_V2)
                        || (r->stream
                            && r->stream->connection->connection->buffered)
#endif
                       )
                    {
                   ...
                   )
                {
                    ...
                    if (n > 0) {
                        ...
                        if (ngx_http_upstream_non_buffered_filter(r, n)
                            == NGX_ERROR)
                        {
                            ngx_http_upstream_finalize_request(r, u, NGX_ERROR);
                            return;
    ...
    if (r->http_version >= NGX_HTTP_VERSION_20) {
        ...
        if (downstream->read->error) {
            ngx_http_upstream_finalize_request(r, u, 0);
            return;
        }

        downstream_buffered = downstream->buffered;

#if (NGX_HTTP_V2)
        if (r->stream) {
            downstream_buffered |= r->stream->connection->connection->buffered;
        }
#endif
        ...
        /*
         * An upstream EOF is a half-close.  Propagate it only after
         * all upstream data has been sent to the client.
         */
            ...
            && !downstream_buffered)
        {
            ...
            if (r->http_version == NGX_HTTP_VERSION_30) {
                rc = ngx_quic_shutdown_stream(r->connection,
                                              NGX_WRITE_SHUTDOWN);
            ...
            ngx_log_debug0(NGX_LOG_DEBUG_HTTP, c->log, 0,
                           "http upstream client write shutdown");
        }

        request_pending = r->reading_body
        ...
        /*
         * A client EOF is a half-close.  Send the upstream TCP FIN
         * only after all client data has been sent to the upstream.
         */
            ...
            && !request_pending)
        {
            ...
            ngx_log_debug0(NGX_LOG_DEBUG_HTTP, c->log, 0,
                           "http upstream peer write shutdown");
        }
        ...
        /*
         * Once both read sides are closed, finalization closes the
         * remaining write side.  Wait until both directions are drained.
         */
    ...
    if (r->http_version >= NGX_HTTP_VERSION_20) {
        if (downstream->data == r
            && ngx_handle_write_event(downstream->write, clcf->send_lowat)
               != NGX_OK)
        {
            ngx_http_upstream_finalize_request(r, u, NGX_ERROR);
            return;
