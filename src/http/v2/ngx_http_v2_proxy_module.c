...
static ngx_int_t ngx_http_v2_proxy_non_buffered_filter(void *data,
    ssize_t bytes);
static ngx_int_t ngx_http_v2_proxy_body_filter(ngx_event_pipe_t *p,
    ngx_buf_t *buf);
static ngx_int_t ngx_http_v2_proxy_process_control_frame(ngx_http_request_t *r,
    ngx_http_v2_proxy_ctx_t *ctx, ngx_buf_t *b);
static ngx_int_t ngx_http_v2_proxy_skip_frame(ngx_http_v2_proxy_ctx_t *ctx,
    ngx_buf_t *b);
...
ngx_http_v2_proxy_handler(ngx_http_request_t *r)
{
    ...
    u->pipe->input_filter = ngx_http_v2_proxy_body_filter;
    u->pipe->input_ctx = r;
...
ngx_http_v2_proxy_create_request(ngx_http_request_t *r)
{
    ...
    if (method.len == 3 && ngx_strncmp(method.data, "GET", 3) == 0) {
        ...
        *b->last++ = ngx_http_v2_inc_indexed(NGX_HTTP_V2_METHOD_INDEX);
        ...
        ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                       "proxy http2 header: \":method: %V\"", &method);
    }
    ...
    if (plcf->proxy_lengths && ctx->ctx.vars.uri.len) {
        ...
        *b->last++ = ngx_http_v2_inc_indexed(NGX_HTTP_V2_PATH_INDEX);
        ...
        ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                       "proxy http2 header: \":path: %V\"", &ctx->ctx.vars.uri);
    ...
    if (!plcf->host_set) {
        *b->last++ = ngx_http_v2_inc_indexed(NGX_HTTP_V2_AUTHORITY_INDEX);
        ...
        ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                       "proxy http2 header: \":authority: %V\"", host);
    }
...
ngx_http_v2_proxy_process_header(ngx_http_request_t *r)
{
    ...
    for ( ;; ) {
        ...
        if (u->peer.connection) {
            ...
            rc = ngx_http_v2_proxy_process_control_frame(r, ctx, b);
            ...
            && ctx->type != NGX_HTTP_V2_CONTINUATION_FRAME)
        {
            ...
            rc = ngx_http_v2_proxy_skip_frame(ctx, b);
...
static ngx_int_t
ngx_http_v2_proxy_non_buffered_filter(void *data, ssize_t bytes)
{
    ngx_http_request_t   *r = data;
    ...
    for ( ;; ) {
        ...
        rc = ngx_http_v2_proxy_process_frames(r, ctx, b);
...
static ngx_int_t
ngx_http_v2_proxy_body_filter(ngx_event_pipe_t *p, ngx_buf_t *b)
{
    ngx_int_t                 rc;
    ngx_buf_t                *buf, **prev;
    ngx_chain_t              *cl;
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "proxy http2 filter bytes:%z", b->last - b->pos);
    ...
    for ( ;; ) {
        ...
        if (rc == NGX_OK) {
            ...
            *prev = buf;
            ...
            ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                           "proxy http2 copy buf %p", buf->pos);
...
static ngx_int_t
ngx_http_v2_proxy_process_control_frame(ngx_http_request_t *r,
    ngx_http_v2_proxy_ctx_t *ctx, ngx_buf_t *b)
{
    ngx_int_t             rc;
    ngx_http_upstream_t  *u;
...
static ngx_int_t
ngx_http_v2_proxy_skip_frame(ngx_http_v2_proxy_ctx_t *ctx, ngx_buf_t *b)
{
    ...
    ngx_http_v2_proxy_ctx_t *ctx, ngx_peer_connection_t *pc)
{
    ...
    if (c == NULL) {
        ctx->connection = ngx_palloc(r->pool, sizeof(ngx_http_v2_proxy_conn_t));
        if (ctx->connection == NULL) {
            return NGX_ERROR;
        }
