...
ngx_http_proxy_create_request(ngx_http_request_t *r)
{
    ...
    if (plcf->host && ngx_http_complex_value(r, plcf->host, &host) != NGX_OK) {
        return NGX_ERROR;
    }

    if (host.len == 0 && plcf->http_version == NGX_HTTP_VERSION_11) {
