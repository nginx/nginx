...
static ngx_command_t  ngx_http_tunnel_commands[] = {
    ...
    { ngx_string("tunnel_socket_rcvbuf"),
      ...
      ngx_http_upstream_socket_buffer_set_slot,
      NGX_HTTP_LOC_CONF_OFFSET,
...
ngx_http_tunnel_create_loc_conf(ngx_conf_t *cf)
{
    ...
    conf->upstream.socket_rcvbuf = NGX_CONF_UNSET;
    conf->upstream.socket_sndbuf = NGX_CONF_UNSET;
...
ngx_http_tunnel_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    ngx_conf_merge_value(conf->upstream.socket_rcvbuf,
                              prev->upstream.socket_rcvbuf, 0);

    ngx_conf_merge_value(conf->upstream.socket_sndbuf,
...
ngx_http_tunnel_pass(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (cf->args->nelts == 1) {
        ngx_str_set(&url, "$host:$request_port");
