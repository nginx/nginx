    ...
    ngx_str_t *key, ngx_uint_t *ep, ngx_uint_t account)
{
    ...
    while (node != sentinel) {
        ...
        if (rc == 0) {
            ...
            excess = lr->excess - ctx->rate * ms / NGX_HTTP_LIMIT_REQ_RATE_SCALE_MULTIPLIER + NGX_HTTP_LIMIT_REQ_RATE_SCALE_MULTIPLIER;
...
ngx_http_limit_req_expire(ngx_http_limit_req_ctx_t *ctx, ngx_uint_t n)
{
    ...
    while (n < 3) {
        ...
        if (n++ != 0) {
            ...
            excess = lr->excess - ctx->rate * ms / NGX_HTTP_LIMIT_REQ_RATE_SCALE_MULTIPLIER;
