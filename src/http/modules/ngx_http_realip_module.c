...
typedef struct {
    ...
    ngx_http_complex_value_t    *source_cv;
...
ngx_http_realip_handler(ngx_http_request_t *r)
{
    ...
    case NGX_HTTP_REALIP_VARIABLE:
        ...
        if (ngx_http_complex_value(r, rlcf->source_cv, &source) != NGX_OK) {
...
ngx_http_realip(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    /*
    * ${} style variables are required, as '{' and '}' are not valid header
    * field name characters.
    */

    if (ngx_strncmp(value[1].data, "${", 2) == 0) {
        rlcf->source_cv = ngx_palloc(cf->pool, sizeof(ngx_http_complex_value_t));
        if (rlcf->source_cv == NULL) {
            return NGX_CONF_ERROR;
        }
        ...
        ccv.cf = cf;
        ccv.complex_value = rlcf->source_cv;
        ccv.value = &value[1];
...
ngx_http_realip_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    if (conf->source_cv == NULL) {
        conf->source_cv = prev->source_cv;
    }
