...
static ngx_http_log_buf_ctx_t *ngx_http_log_get_buffer(ngx_open_file_t *file);
...
ngx_http_log_handler(ngx_http_request_t *r)
{
    ...
    for (l = 0; l < lcf->logs->nelts; l++) {
        ...
        buffer = log[l].file ? ngx_http_log_get_buffer(log[l].file) : NULL;
...
static ngx_http_log_buf_ctx_t *
ngx_http_log_get_buffer(ngx_open_file_t *file)
{
    ngx_pool_t              *pool;
    ngx_http_log_buf_t      *buffer;
    ngx_http_log_buf_ctx_t  *ctx;
    ...
    ctx = ngx_pcalloc(pool, sizeof(ngx_http_log_buf_ctx_t));
    if (ctx == NULL) {
        return NULL;
    }
    ...
    if (buffer->flush) {
        ...
        ctx->event->data = file;
        ctx->event->handler = ngx_http_log_flush_handler;
...
ngx_http_log_flush(ngx_open_file_t *file, ngx_log_t *log)
{
    ...
    buffer = ngx_http_log_get_buffer(file);
    if (buffer == NULL) {
        return;
    }
...
ngx_http_log_open_file_cache(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    llcf->open_file_cache = ngx_open_file_cache_init(cf, max, inactive);
